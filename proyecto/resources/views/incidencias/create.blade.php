...
<div class="card shadow-sm">
    <div class="card-header bg-primary text-white"><h5 class="mb-0">Nueva Incidencia</h5></div>
    <div class="card-body">
        <form action="{{ route('incidencias.store') }}" method="POST">
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select name="cliente_id" class="form-select @error('cliente_id') is-invalid @enderror" required>
                        ...
                        @foreach($clientes as $c)<option value="{{ $c->id }}" {{ old('cliente_id')==$c->id?'selected':'' }}>{{ $c->nombre }}</option>@endforeach
                    </select>
            ...
            <div class="row">
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <select name="provincia_codigo" class="form-select @error('provincia_codigo') is-invalid @enderror" required>
                        <option value="">Seleccione...</option>
                        @foreach($provincias as $p)<option value="{{ $p->codigo_ine }}" {{ old('provincia_codigo')==$p->codigo_ine?'selected':'' }}>{{ $p->nombre }}</option>@endforeach
                    </select>
            ...
            <div class="mb-3">
                ...
                <select name="operario_id" class="form-select @error('operario_id') is-invalid @enderror" required>
                    <option value="">Seleccione...</option>
                    @foreach($operarios as $o)<option value="{{ $o->id }}" {{ old('operario_id')==$o->id?'selected':'' }}>{{ $o->nombre }}</option>@endforeach
                </select>
